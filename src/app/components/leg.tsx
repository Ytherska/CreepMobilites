...
const Leg = React.memo(function({ from, to, line, stops, segments, time }: LegProp) {
    ...
    return (
        <div className="route-step">
            Prendre le <span className={`line-badge ${isHSR && "hsr-badge"}`} style={{ backgroundColor: line.colour }}>{line.id} ({line.name})</span> de <strong className="transfer-station">{from}</strong> à <strong className="transfer-station">{to}</strong>

            {!isHSR && stops.length > 2 && <div className="station-count">Via : {stops.slice(1, -1).join(", ")}</div>}
